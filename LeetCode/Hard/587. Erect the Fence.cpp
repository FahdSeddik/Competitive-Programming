...
public:
	...
	bool operator < (point &points) {
		if (points.x == x) return y < points.y;  // if x pts are same then compare according to y
		else return x < points.x;
	}
	bool operator == (point &points) {  // operator overloading for unique() func
